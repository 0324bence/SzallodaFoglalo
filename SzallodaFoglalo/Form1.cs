...
namespace SzallodaFoglalo
{
    public partial class Form1 : Form
    {
        ...
        {   
            ...
            fajlutvonal = AppDomain.CurrentDomain.BaseDirectory + @"\Lists\" + evjarat + ".txt";
        ...
        private void cell_click(object sender, DataGridViewCellEventArgs kivalasztott) {
            ...
            {
                MessageBox.Show("Ez a dátum már foglalt vagy  a hónap határain kívűl esik!\n             Kérem válasszon másikat!");
            }
        }
        ...
        {
            bool kitoltottemindet = !string.IsNullOrWhiteSpace(foglaloneveBox.Text) && letszam.SelectedItem != null && (radioBtnZero.Checked || radioBtnOne.Checked);
            foglalGomb.Enabled = kitoltottemindet;
        }
        ...
        {   
            ...
            if (length != 0)
            {
                ...
                StreamWriter ir = new StreamWriter(AppDomain.CurrentDomain.BaseDirectory + @"\Lists\Bevetel_" + evjarat+".txt");
                for (int i = 0; i < length; i++)
                {
                    #region Tavasz
            ...
            {
                MessageBox.Show("Ebben az évben még nem történt foglalás");
            }
